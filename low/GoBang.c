...
void disDeep()
{
	...
	for (i=0;i<15;i++)
	{
		for (j=0;j<15;j++)
		{
			...
			if ((play_sum[i][j]%10)>=(max_p%10))
			{
				if ((play_sum[i][j]%10) == (max_p%10))
				{
					if ((ai_sum[x_p][y_p]%10) < (ai_sum[i][j]%10))
					{
						x_p = i;
						y_p = j;
	...
	if ((max_p%10) == (max_a%10)&&(max_a%10) == 1)
	{
		while (1){
		if (go[x_w][y_w] == 0){
			go[x_w][y_w] = 2;
	...
	if ((deepl(x_a, y_a, 1)%10) >= (max_p%10))
	{
		x_p = x_a;
		y_p = y_a;
	}
	//优先级待完成，目前只有围追堵截（还行）
	if ((max_a%10)>(max_p%10))
		flag=1;
	else if ((max_a%10)<(max_p%10))
		flag=0;
	else if ((max_a%10)==(max_p%10))
	{
		if ((max_a%10)>=4&&max_a<10&&max_p<10)
		{
			flag = 1;
		}
		else if ((max_a%10)>=4&&max_a>10&&max_p>10)
		{
			flag = 1;
		}
		else if ((max_a%10)>=4&&max_a<10&&max_p>10)
...
int deep(int cx, int cy, int num, int color)
{
	...
	rx = x;
	ry = y;
	if (go[lx+a[num].x][ly+a[num].y]==0&&go[rx-a[num].x][ry-a[num].y]==0)
		count = count + 11;//提高了一个优先级
	#ifdef DEBUG
	printf("\n");
	#endif
	//对边界情况的判断
	if ((cx==0||cx==14||cy==0||cy==14)&&(count%10)<4)
		count = 1;
	if ((cx==1||cx==13||cy==1||cy==13)&&(count%10)<3)
		count = 1;
	//if (emptyl>0&&emptyr>0&&count>3)
	//	count++;
	//if (count>5)
	//{
	//	if (color==1)
